...
GOOGLE_BACKUP_INTERVAL_MINUTES = max(
    1.0, float(os.getenv("GOOGLE_BACKUP_INTERVAL_MINUTES", "5") or 5)
)
...
def run_auto_backup_worker():
    """Refresh the same Google Sheet periodically after bot data changes."""
    ...
    interval_seconds = GOOGLE_BACKUP_INTERVAL_MINUTES * 60
    while True:
...
def check_binance_payment(transaction_id):
    ...
    request_url = f"{BINANCE_PAY_API_ENDPOINT}?{query_string}&signature={signature}"
    request = Request(
        request_url,
    ...
    try:
        ...
        transactions = payload.get("data", []) if isinstance(payload, dict) else payload
        for item in transactions or []:
            order_id = str(
                item.get("orderId", "") or item.get("transactionId", "")
            ).strip()
            if order_id != str(transaction_id).strip():
                continue
        ...
        logging.error("Binance API HTTP error: %s", error.code)
        return False, 0.0, "Binance API error বা API permission সমস্যা।"
...
async def send_admin_panel(update: Update):
    ...
    text = (
        ...
        f"📢 <b>Log Group ID:</b> <code>{log_group}</code>"
    )
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'awaiting_trxid':
        ...
        for admin_id in get_all_admin_ids():
            try:
                await context.bot.send_message(chat_id=admin_id, text=pe(admin_msg), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            except Exception:
                pass
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id) and data == "admin_backup":
        ...
        try:
            ...
            logging.error("Google Sheet backup failed: %s", error)
            await query.edit_message_text(
                pe(
                    "❌ <b>Backup করা যায়নি।</b>\n\n"
                    "Render env-এ GOOGLE_SHEET_ID এবং "
                    "GOOGLE_SERVICE_ACCOUNT_JSON ঠিক আছে কিনা দেখুন।"
                ),
