...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'awaiting_trxid':
        ...
        if method.lower() == 'binance':
            ...
            manual_binance_fallback = False
            if not is_valid or paid_amount < dep_usdt:
                if "restricted location" in verify_message.lower():
                    # Binance is rejecting the hosting server's IP, not the
                    # payment itself. Never auto-credit an unverified payment;
                    # send it through the existing admin-review flow instead.
                    manual_binance_fallback = True
                    method = "Binance (manual verification)"
                    conn = get_db()
                    cursor = conn.cursor()
                    await context.bot.edit_message_text(
                        ...
                        text=pe(
                            "⚠️ <b>Binance auto-verification সাময়িকভাবে unavailable</b>\n\n"
                            "আপনার payment request হারায়নি। Admin manually verify করে "
                            "approve করবেন।"
                        ),
                        parse_mode="HTML",
                    )
            ...
            if not manual_binance_fallback:
