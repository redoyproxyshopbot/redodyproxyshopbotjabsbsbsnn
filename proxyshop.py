...
try:
    SUPER_ADMIN_ID = int(_admin_id_value)
except ValueError:
    SUPER_ADMIN_ID = 0
...
BINANCE_PAY_ID = os.getenv("BINANCE_PAY_ID", "1261150963").strip()
BINANCE_PAY_API_ENDPOINT = os.getenv(
    "BINANCE_PAY_API_ENDPOINT",
    "https://api.binance.com/sapi/v1/pay/transactions",
).strip()
try:
    BINANCE_RECV_WINDOW = max(
        1000, min(60000, int(os.getenv("BINANCE_RECV_WINDOW", "5000")))
    )
...
MONGODB_DATABASE = os.getenv("MONGODB_DATABASE", "gx_shop_bot").strip()
RUNTIME_DB_PATH = os.getenv("RUNTIME_DB_PATH", "/tmp/gx_shop_runtime.db")
...
def init_db():
    ...
    default_methods = [
        ...
        ('Binance', BINANCE_PAY_ID or '1261150963', 0.1, 'binance')
    ]
...
def check_binance_payment(transaction_id):
    ...
    submitted_id = str(transaction_id or "").strip()
    if not submitted_id:
        return False, 0.0, "Transaction ID দেওয়া হয়নি।"
    ...
    try:
        ...
        for item in transactions or []:
            ...
            # Binance can expose the same payment with different identifiers
            # depending on the account/API version. The old implementation
            # checked only orderId first, so a valid transactionId was missed
            ...
            if status in {"FAILED", "CANCELED", "CANCELLED", "REJECTED", "ERROR"}:
                return False, paid_amount, f"Binance transaction status {status} গ্রহণযোগ্য নয়।"
            if paid_amount <= 0:
                return False, 0.0, "Binance transaction amount সঠিক নয়।"
        ...
        try:
            ...
            error_body = ""
        logging.error("Binance API HTTP error: %s %s", error.code, error_body)
        try:
            error_payload = json.loads(error_body)
        except (TypeError, ValueError, json.JSONDecodeError):
            error_payload = {}
        api_code = str(error_payload.get("code", "")).strip()
        api_message = str(error_payload.get("msg", "")).strip()
        if api_code or api_message:
            detail = f"Binance error {api_code}: {api_message}".strip(": ")
            return False, 0.0, detail[:240]
        return False, 0.0, f"Binance HTTP error ({error.code}) বা API permission সমস্যা।"
...
async def send_admin_panel(update: Update):
    ...
    keyboard = [
        ...
        [
            pbtn("GX Control", icon="gear", style="danger", callback_data='admin_gx_control')
        ],
        [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_msg = f"👋 <b>GX SHOP</b> এ আপনাকে স্বাগতম, {user.first_name}!\n\nনিচের মেনু থেকে আপনার সেবাটি বাছুন:"
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id) and state in ['admin_add_balance', 'admin_cut_balance']:
        ...
        await update.message.reply_text(pe(msg), reply_markup=InlineKeyboardMarkup([[pbtn("Back to GX Control", icon="back", callback_data="admin_gx_control")]]), parse_mode="HTML")
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id) and data == "admin_del_prod_prompt":
        ...
        if not cats:
            await query.edit_message_text(pe("❌ <b>কোনো ক্যাটাগরি নেই!</b>"), reply_markup=InlineKeyboardMarkup([[pbtn("Back", icon="back", callback_data="admin_gx_control")]]), parse_mode="HTML")
            return
        ...
        keyboard.append([pbtn("Back", icon="back", callback_data="admin_gx_control")])
    ...
    if is_admin(user.id) and data == "delitem_confirm":
        ...
        if not title:
            await query.edit_message_text(pe("❌ কিছু ভুল হয়েছে, আবার চেষ্টা করুন।"), reply_markup=InlineKeyboardMarkup([[pbtn("Back", icon="back", callback_data="admin_gx_control")]]), parse_mode="HTML")
            return
        ...
        await query.edit_message_text(pe(f"✅ সফলভাবে <b>{title}</b> প্রোডাক্টটি স্টক থেকে ডিলিট করা হয়েছে!"), reply_markup=InlineKeyboardMarkup([[pbtn("Back to GX Control", icon="back", callback_data="admin_gx_control")]]), parse_mode="HTML")
        return

    # GX Control Sub-Menu Handler
    if is_admin(user.id) and data == "admin_gx_control":
        context.user_data.pop('state', None)
        ref_bonus = get_setting('referral_commission')
        usdt_rate = get_setting('usdt_rate') or "120"
    ...
    if is_admin(user.id) and data == "admin_balance_menu":
        ...
        keyboard = [
            ...
            [
                pbtn("Back to GX Control", icon="back", callback_data="admin_gx_control")
            ]
        ]
        await query.edit_message_text(
    ...
    if is_admin(user.id) and data == "admin_set_usdt_rate":
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[pbtn("Cancel", icon="close", style="danger", callback_data="admin_gx_control")]]),
            parse_mode="HTML"
        )
        return
    ...
    if data == "check_join":
        ...
        if is_joined:
            ...
            await context.bot.send_message(
                chat_id=user.id,
                text=pe(f"👋 <b>GX SHOP</b> এ আপনাকে স্বাগতম, {user.first_name}!"),
    ...
    if is_admin(user.id) and data == 'admin_set_payment':
        ...
        keyboard.append([pbtn("Back to GX Control", icon="back", callback_data='admin_gx_control')])
        ...
        await query.edit_message_text(pe("🎁 রেফারেল কমিশন কত পার্সেন্ট (<b>%</b>) দিতে চান? (শুধু সংখ্যা লিখুন, যেমন: 10):"), reply_markup=InlineKeyboardMarkup([[pbtn("Cancel", icon="close", style="danger", callback_data="admin_gx_control")]]), parse_mode="HTML")
        return
    ...
    if is_admin(user.id) and data == 'admin_fj_sys':
        ...
        await query.edit_message_text(pe("💬 নতুন <b>Support Username / Link</b> পাঠান:"), reply_markup=InlineKeyboardMarkup([[pbtn("Cancel", icon="close", style="danger", callback_data="admin_gx_control")]]), parse_mode="HTML")
        return
...
@health_app.get("/")
def health_home():
    return jsonify({"status": "ok", "service": "gx-shop-bot"})
